...
**Second attempt: a structural fix, not yet tested.** The judge's output schema now requires a `"claims"` array — a list of every concrete, checkable claim extracted from each step, tagged by step index — that has to come *before* `has_failure` in the response. This forces the extraction pass to be a visible, required part of the output rather than an internal instruction the model can silently skip. [debug_judge.py](debug_judge.py) (new) inspects the full raw verdict for exactly this reason: it shows the extracted `claims` list, not just the final answer, so a miss can be diagnosed as "didn't extract the conflicting claims at all" vs. "extracted both but failed to compare them." Not tested yet — the first attempt to run it hit the same daily quota wall before a single call completed.

## Where this goes next

- **Actually test the structural fix** — `python3 debug_judge.py real_traces/multi_agent_research_live.json` costs one call and shows whether the claims array now captures both the Analyst's and the Critic's conflicting Nobel Prize statements, which tells us whether this is an extraction problem or a comparison problem.
- **Get more completed runs either way** before trusting the "prompt-only doesn't work" conclusion much further — 5 total data points across two prompt versions is enough to be suspicious, not enough to be certain.
