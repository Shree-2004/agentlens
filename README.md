...
**First prompt-fix attempt: inconclusive.** `judge.py`'s prompt now explicitly says contradictions can be prose-buried, not just structured, and to extract concrete claims from every step before comparing. Re-tested against the same real trace 3 times; only 1 call actually completed (quota + "high demand" errors took the other 2), and that one run *still* missed the contradiction. One data point isn't enough to call the prompt fix a failure, but it's not a win yet either — needs more real runs before concluding anything.

## Where this goes next

- **Get more real runs on the revised prompt** before judging whether it worked — current status is 1 inconclusive data point post-fix, same miss as before.
- **If the prompt fix doesn't pan out**, consider whether this needs a structurally different approach (e.g. asking the judge to first list out extracted claims per step as an intermediate output, not just as an instruction) rather than just stronger wording.
