...
**First prompt-fix attempt: looks like it didn't work.** `judge.py`'s prompt now explicitly says contradictions can be prose-buried, not just structured, and to extract concrete claims from every step before comparing. Re-tested against the same real trace across two sessions (limited by quota to 1 completed call each time) — both post-fix runs still missed the contradiction. That's 5/5 misses total across both prompt versions (3 before the fix, 2 after). Still a small sample, but consistent enough to draw a tentative conclusion: telling the judge to extract claims first, as an instruction, doesn't reliably make it actually do that as a distinct step. It's a one-pass "read everything, then answer" call — a wordier instruction may not change *how* it processes the input, just what it's told to prioritize.

## Where this goes next

- **Try a structural fix instead of a wordier prompt.** Make claim extraction a required *output field* — have the judge list concrete claims per step in its own part of the JSON response before it's allowed to conclude has_failure — rather than an instruction it can silently skip. Forcing the intermediate step into the output is a stronger guarantee than asking nicely for it in the system prompt.
- **Get more completed runs either way** before trusting this conclusion much further — 5 total data points across two prompt versions is enough to be suspicious, not enough to be certain.
